...
/**
 * Imprime a proposta num iframe oculto.
 *
 * ------------------------------------------------------------------
 * POR QUE CONFERIR ANTES DE IMPRIMIR
 * ------------------------------------------------------------------
 * Este PDF já saiu em branco duas vezes, por motivos diferentes (uma delas,
 * `opacity: 0` no iframe). A causa raiz comum é sempre a mesma: mandávamos
 * `print()` confiando que o documento estava pronto — e quando não estava, o
 * corretor recebia uma folha vazia sem nenhum aviso.
 *
 * Agora nada é impresso sem CONFIRMAR que a folha (`.sheet`) existe e tem
 * altura. Se o `srcdoc` não renderizar, tentamos `document.write`; se ainda
 * assim não vier, abrimos numa aba nova; e se nem isso, a Promise REJEITA para
 * a tela poder avisar. Um erro visível é muito melhor que um PDF em branco.
 */
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const hasContent = (): boolean => {
      try {
        ...
        const sheet = cdoc?.querySelector?.('.sheet');
    ...
    const doPrint = () => {
      ...
      if (!win || !win.print) {
        finish(new Error('Este navegador não permite imprimir a proposta.'));
        return;
      }
    ...
    /** Último recurso: aba nova com a proposta, para o corretor imprimir de lá. */
    ...
    const waitForContent = () => {
      ...
      // (a foto da construtora vai embutida e engorda o HTML).
      ...
      if (waited >= READY_TIMEOUT_MS) {
        ...
        finish(new Error('Não foi possível montar a proposta para impressão.'));
        return;
      }
