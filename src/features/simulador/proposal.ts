...
export async function generateProposal(ctx: ProposalContext): Promise<void> {
  ...
  if (Platform.OS === 'web') {
    // A conferência de renderização e os planos B da impressão na web moram em
    // `features/pdf/imprimir.ts` — o simulador de financiamento usa os mesmos.
    await imprimirHtmlNaWeb(html, proposalFileName(ctx));
    return;
  }
