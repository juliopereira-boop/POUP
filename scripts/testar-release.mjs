...
const valid = { EXPO_PUBLIC_SUPABASE_URL: 'https://project.supabase.co',
  EXPO_PUBLIC_SUPABASE_ANON_KEY: 'sb_publishable_test-fixture',
  EXPO_PUBLIC_APP_URL: 'https://poup.example.com',
  EXPO_PUBLIC_STRIPE_PRICE_START: 'price_start123', EXPO_PUBLIC_STRIPE_PRICE_PRO: 'price_pro123' };
let count = 0;
function check(patch, expected, context = 'web') {
  ...
  const result = spawnSync(process.execPath, ['scripts/checar-ambiente.mjs', context, '--exigir'], {
    env: { ...env, ...valid, ...patch }, encoding: 'utf8',
  });
  assert.equal(result.status, expected, result.stderr); count++;
  return result;
}
check({}, 0);
check({ EXPO_PUBLIC_STRIPE_PRICE_START: '', EXPO_PUBLIC_STRIPE_PRICE_PRO: '' }, 0, 'loja');
for (const url of ['http://localhost:8081', 'https://127.0.0.1', 'https://192.168.1.3', 'invalid', 'https://user:password@poup.com', 'https://poup.com?token=abc']) {
  check({ EXPO_PUBLIC_APP_URL: url }, 1);
}
check({ EXPO_PUBLIC_SUPABASE_URL: 'https://placeholder.supabase.co' }, 1);
check({ EXPO_PUBLIC_STRIPE_PRICE_START: 'price_...' }, 1);
check({ EXPO_PUBLIC_STRIPE_PRICE_START: valid.EXPO_PUBLIC_STRIPE_PRICE_PRO }, 1);
